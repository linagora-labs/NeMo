...
class AudioPerceptionModule(NeuralModule, Exportable):
    ...
    def encoder_frame_duration(self) -> float:
        ...
        (before the modality adapter). This is the cadence ROTE uses to map frame indices to
        absolute time, so it excludes the adapter subsampling factor included in
    ...
    def _apply_rote(self, encoder_emb, time_offset=None):
        ...
        ``time_offset[b] + (frame_idx + 0.5) * encoder_frame_duration`` (frame-center
        convention). When the encoder produces per-layer activations (Qformer / MultiLayer
        adapters), ROTE is applied to each layer's features.
        """
